...
public class ConservationFormatter {
    ...
    static <T> void printResultWithAlignment(AminoAcidMatrix alignment, T tag,
	    ...
	    int resultPrecision, String outputFile) {
    ...
    static void formatResults(Map<Method, double[]> scores, String outFilePath,
	    String format, AminoAcidMatrix alignment) {
	...
	if (outFilePath == null) {
		...
		if (print != null) {
			...
			ConservationAccessory.printArrayOfDouble(scores
				.get(key), print, precision);
		...
		if (Format.getFormat(format) == Format.RESULT_WITH_ALIGNMENT) {
			...
			while (itr.hasNext()) {
			    ...
			    ConservationAccessory.printArrayOfDoubleFieldWidth(
				    scores.get(key), print, precision,
				    fieldWidth);
