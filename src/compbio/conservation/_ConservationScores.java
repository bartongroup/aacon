...
public class _ConservationScores {
	...
	if (matrix.getTotalAcidsFreqByCol().get(columnNr).size() == 1
		&& matrix.getTotalAcidsFreqByCol().get(columnNr).containsKey(
			'-') == false) {
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
	    ...
	    result[i] = matrix.getInverseMatrix()[i].length
		    * (double) numberOfAcidsNoGap(i)
		    / (double) mostCommonNumber(i);
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
	    ...
	    result[i] = ShannonEnthropy.ShannonLn(matrix
		    .getTotalAcidsFreqByCol().get(i),
		    matrix.getInverseMatrix()[i].length)
		    * normal;
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
	    ...
	    result[i] = Math.pow(2.0, ShannonEnthropy.ShannonLog2(matrix
		    .getTotalAcidsFreqByCol().get(i),
		    matrix.getInverseMatrix()[i].length)) * 6.0;
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
	    ...
	    result[i] = -(ShannonEnthropy.ShannonLn(matrix
		    .totalAcidsFrequency(), matrix.numberOfColumns()
		    * matrix.numberOfRows()))
		    - (-ShannonEnthropy.ShannonLn(matrix
			    .getTotalAcidsFreqByCol().get(i), matrix
	...
	if (normalize) {
	    ...
	    double[] normalized = ConservationAccessory
		    .inversedNormalize01(smallestSetSize);
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
	    ...
	    acidsMapNoGaps = new HashMap<Character, Integer>(matrix
		    .getTotalAcidsFreqByCol().get(i));
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
		...
		if (matrix.getInverseMatrix()[i][a] != '-') {
			...
			if (matrix.getInverseMatrix()[i][b] != '-') {
			    ...
			    double pairScore = ConservationMatrices.BlosumPair(
				    matrix.getInverseMatrix()[i][a], matrix
					    .getInverseMatrix()[i][b]);

			    double aSelf = ConservationMatrices.BlosumPair(
				    matrix.getInverseMatrix()[i][a], matrix
					    .getInverseMatrix()[i][a]);

			    assert aSelf > 0;

			    double bSelf = ConservationMatrices.BlosumPair(
				    matrix.getInverseMatrix()[i][b], matrix
					    .getInverseMatrix()[i][b]);
	    ...
	    finalSum[i] = blosumSum[i]
		    * (2.0 / (matrix.getInverseMatrix()[i].length * (matrix
			    .getInverseMatrix()[i].length - 1)));
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
	    ...
	    int arrayLength = matrix.getTotalAcidsFreqByCol().get(i).keySet()
		    .size();
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
		...
		for (int b = 0; b < alp.length; b++) {
		    ...
		    points[a][b] = ConservationMatrices.BlosumPair(matrix
			    .getInverseMatrix()[i][a], alp[b]);
	    ...
	    meanPoint = ConservationAccessory.multPointByScalar(sum,
		    1.0 / (double) matrix.getInverseMatrix()[i].length);

	    for (int c = 0; c < matrix.getInverseMatrix()[i].length; c++) {

		distance = distance
			+ ConservationAccessory.pointDistance(points[c],
				meanPoint);
	    ...
	    result[i] = nonGapsFraction * 1.0
		    / matrix.getInverseMatrix()[i].length * distance;
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
		...
		while (itr2.hasNext()) {
		    ...
		    result[i] = result[i]
			    + ((((double) matrix.getTotalAcidsFreqByCol()
				    .get(i).get(key1)
				    / (double) matrix.getInverseMatrix()[i].length
				    * (double) matrix.getTotalAcidsFreqByCol()
					    .get(i).get(key2) / (double) matrix
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
	    ...
	    Set<Character> acInKeys = matrix.getTotalAcidsFreqByCol().get(i)
		    .keySet();
		...
		while (acInKeysItr.hasNext()) {
			...
			if (count == null) {
			    ...
			    setsFreq.put(mirnyKey, count
				    + matrix.getTotalAcidsFreqByCol().get(i)
					    .get(acInKey));
		...
		System.out.println(i + " " + "Sets tester1 " + " key "
			+ setFreqKey + " value " + setsFreq.get(setFreqKey));
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
		...
		System.out.println("Sets tester " + " key " + setFreqKey
			+ " value " + setsFreq.get(setFreqKey));
		...
		double piAve = (double) matrix.totalAcidsWillSets().get(
			setFreqKey)
			/ matrix.numberOfRows();
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
		...
		for (int b = a + 1; b < matrix.getInverseMatrix()[i].length; b++) {
		    ...
		    double disIJ = ConservationMatrices.dissimilarity(matrix
			    .getInverseMatrix()[i][a], matrix
			    .getInverseMatrix()[i][b]);

		    double disJI = ConservationMatrices.dissimilarity(matrix
			    .getInverseMatrix()[i][b], matrix
			    .getInverseMatrix()[i][a]);
	...
	for (int i = 0; i < matrix.getInverseMatrix().length; i++) {
		...
		for (int b = a + 1; b < matrix.getInverseMatrix()[i].length; b++) {
		    ...
		    sum = sum
			    ...
			    * ConservationMatrices.pam250Pair(matrix
				    .getInverseMatrix()[i][a], matrix
				    .getInverseMatrix()[i][b]);
		}
